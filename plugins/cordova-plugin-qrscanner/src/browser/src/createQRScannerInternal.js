...
module.exports = function(){
  ...
  function getTempCanvasAndContext(videoElement){
    ...
    tempCanvas.height = 720; //camera.height;
    tempCanvas.width = 1024; // camera.width;
    var tempCanvasContext = tempCanvas.getContext('2d');
    tempCanvasContext.drawImage(videoElement, 0, 0, tempCanvas.width, tempCanvas.height);
    return {
      canvas: tempCanvas,
  ...
  function scan(success, error) {
    ...
    show(function(ignore){
      ...
      thisScanCycle = function(){
        let xyz = getCurrentImageData(video)
        scanWorker.postMessage(xyz);
